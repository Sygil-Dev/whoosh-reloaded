from whoosh_reloaded import analysis, fields, query
from whoosh_reloaded.compat import u, text_type
from whoosh_reloaded.qparser import default, plugins


def test_whitespace():
    p = default.QueryParser("t", None, [plugins.WhitespacePlugin()])
    assert (
        repr(p.tag("hello there amiga"))
...
def test_groups():
    ...
    assert (
        repr(ns)
        == "<AndGroup <None:'a'>, <AndGroup <AndGroup <None:'b'>, <None:'c'>>, <None:'d'>>, <None:'e'>>"
    )
...
def test_fieldnames():
    p = default.QueryParser(
        "t",
        None,
        [plugins.WhitespacePlugin(), plugins.FieldsPlugin(), plugins.GroupPlugin()],
    )
    ns = p.process("a:b c d:(e f:(g h)) i j:")
    assert (
        repr(ns)
        == "<AndGroup <'a':'b'>, <None:'c'>, <AndGroup <'d':'e'>, <AndGroup <'f':'g'>, <'f':'h'>>>, <None:'i'>, <None:'j:'>>"
    )
...
def test_boost():
    p = default.QueryParser(
        "t",
        None,
        [plugins.WhitespacePlugin(), plugins.GroupPlugin(), plugins.BoostPlugin()],
    )
    ns = p.tag("a^3")
    ...
    assert (
        repr(p.process("a (b c)^2.5"))
        == "<AndGroup <None:'a'>, <AndGroup <None:'b'>, <None:'c'> ^2.5>>"
    )
    assert (
        repr(p.process("a (b c)^.5 d"))
        == "<AndGroup <None:'a'>, <AndGroup <None:'b'>, <None:'c'> ^0.5>, <None:'d'>>"
    )
...
def test_singlequotes_query():
    ...
    assert q.__class__ == query.And
    assert len(q) == 6
    assert [t.text for t in q.subqueries] == [
        "I",
        "don't",
        "like",
        "W's",
...
def test_empty_ranges():
    schema = fields.Schema(name=fields.TEXT, num=fields.NUMERIC, date=fields.DATETIME)
    qp = default.QueryParser("text", schema)
...
def test_multitoken_with_factory():
    from whoosh_reloaded.qparser.syntax import OrGroup
