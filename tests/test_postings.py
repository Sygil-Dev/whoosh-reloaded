...
def test_position_postings():
    ...
    assert _roundtrip(content, Positions(), "frequency") == [
        ("alfa", 3),
        ("bravo", 2),
        ("charlie", 1),
    ]
...
def test_character_postings():
    ...
    assert _roundtrip(content, Characters(), "positions") == [
        ("alfa", [0, 4, 5]),
        ("bravo", [1, 3]),
        ("charlie", [2]),
    ]
    assert _roundtrip(content, Characters(), "frequency") == [
        ("alfa", 3),
        ("bravo", 2),
        ("charlie", 1),
    ]
...
def test_posboost_postings():
    ...
    assert _roundtrip(content, pbs, "positions", ana) == [
        ("alfa", [0, 4, 5]),
        ("bravo", [1, 3]),
        ("charlie", [2]),
    ]
    assert _roundtrip(content, pbs, "frequency", ana) == [
        ("alfa", 3),
        ("bravo", 2),
        ("charlie", 1),
    ]
...
def test_charboost_postings():
    ...
    assert _roundtrip(content, cbs, "position_boosts", ana) == [
        ("alfa", [(0, 2), (4, 1), (5, 1)]),
        ("bravo", [(1, 0.1), (3, 0.5)]),
        ("charlie", [(2, 2)]),
    ]
    assert _roundtrip(content, cbs, "characters", ana) == [
        ("alfa", [(0, 0, 4), (4, 37, 41), (5, 42, 46)]),
        ("bravo", [(1, 7, 12), (3, 27, 32)]),
        ("charlie", [(2, 17, 24)]),
    ]
    assert _roundtrip(content, cbs, "positions", ana) == [
        ("alfa", [0, 4, 5]),
        ("bravo", [1, 3]),
        ("charlie", [2]),
    ]
    assert _roundtrip(content, cbs, "frequency", ana) == [
        ("alfa", 3),
        ("bravo", 2),
        ("charlie", 1),
    ]
